...
export async function syncToAgents(syncEngine: SyncEngine): Promise<void> {
  ...
  const totalFiles = result.agentResults.reduce((sum, r) => sum + r.filesWritten.length, 0);
  const totalErrors = result.agentResults.reduce((sum, r) => sum + r.errors.length, 0);
  ...
  if (totalErrors > 0) {
    ...
    vscode.window.showInformationMessage(`Sync complete: ${totalFiles} files written.`);
  } else {
